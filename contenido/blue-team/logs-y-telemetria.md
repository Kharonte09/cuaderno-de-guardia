...
> [!IMPORTANT]
> **4104 es el evento más valioso de todo Windows para detección.** Registra el bloque de script tal como se ejecuta, así que ve el contenido aunque venga en Base64. Actívalo por GPO: *Administrative Templates → Windows Components → Windows PowerShell → Turn on PowerShell Script Block Logging*.

### Sysmon

Sysmon no viene instalado, y es lo que convierte un Windows en un endpoint monitorizable de verdad:
...
> [!TIP]
> El evento **1 de Sysmon con el proceso padre** es la base de la mayoría de detecciones buenas. `winword.exe` → `powershell.exe` no tiene explicación legítima en casi ninguna organización.
...
| **DNS** | Qué dominios se resuelven — imprescindible, y muy barato de recoger |
...
Por orden de valor por euro invertido:

1. **Sysmon** (eventos 1, 3, 7, 8, 10, 11, 22) — gratis, y transforma la visibilidad.
...
6. **Identidad cloud** — incluido en la licencia, casi siempre sin usar.
7. EDR — el gasto grande, pero el que más cubre.

> [!WARNING]
> Recoger sin retención suficiente es tirar el dinero. Los atacantes llevan de media semanas dentro antes de ser detectados: con **7 días de retención** no podrás reconstruir nada. El mínimo razonable son 90 días en caliente y un año en frío para los logs clave.

## Higiene de la recolección

- **Sincroniza los relojes** (NTP) en toda la flota. Sin esto, ninguna línea temporal es fiable.
...
- **Monitoriza la recolección misma**: una fuente que deja de enviar logs es una alerta, no un silencio. El atacante que apaga el agente cuenta con que nadie lo note.
